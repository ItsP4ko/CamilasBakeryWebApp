...
import { actualizarEstadoPedido, actualizarPedidoEncabezado, } from "../api/pedidos";
...
export const useUpdatePedido = () => {
    ...
    return useMutation({
        mutationFn: async (pedido) => {
            ...
            if (pedido._cambioMetodo)
                body.MetodoDePago = pedido.metodoDePago;
            if (pedido._cambioNota)
                body.Nota = pedido.nota;
            if (pedido._cambioEstado)
                body.Estado = pedido.estado;
        ...
        onMutate: async (pedidoActualizado) => {
            ...
            queryClient.setQueriesData({ queryKey: queryKeys.pedidos.all }, (oldData) => {
                if (!oldData)
                    return oldData;
                return {
                    ...oldData,
