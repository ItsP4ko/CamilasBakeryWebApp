...
export class ApiError extends Error {
    constructor(status, title, detail, errors, traceId) {
        super(detail ?? title);
        this.status = status;
        this.title = title;
        this.detail = detail;
    ...
    static fromAxiosError(error) {
        ...
        return new ApiError(data?.status ?? error.response?.status ?? 0, data?.title ?? 'Error inesperado', data?.detail, data?.errors, data?.extensions?.traceId);
    }
}
// ─── Cliente Axios ───────────────────────────────────────────────────────────
...
api.interceptors.response.use((response) => response, (error) => {
    ...
    if (error.request) {
        ...
        return Promise.reject(new ApiError(0, 'Sin conexión', 'No se pudo conectar con el servidor. Verificá tu conexión a internet.'));
    }
